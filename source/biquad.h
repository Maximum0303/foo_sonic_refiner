...
public:
    ...
    void set_low_shelf(double sample_rate, double frequency_hz, double gain_db) noexcept {
    ...
    void set_high_shelf(double sample_rate, double frequency_hz, double gain_db) noexcept {
        if (!prepare_frequency(sample_rate, frequency_hz)) {
            set_identity();
            return;
        }
